...
def estilos_globales():
    estilos = """
    <style>
        /* Aumentamos padding superior para que el logo no se corte */
    ...
    <div class="footer-credits">
        <div style="font-weight: bold; color: #002F6C;">Hecho por: Guevara Julián, Jácome Dayana, Lanchimba Michael, Pulamarin Lenin.</div>
...
def interfaz_gestor_archivos():
    ...
    # ... (Misma lógica de gestión de archivos)
    ...
    if uploaded_files:
        if st.button("Procesar Documentos", type="primary"):
            ...
            st.rerun()
    # Listado de archivos para borrar...
...
def interfaz_chat():
    ...
    # === ENCABEZADO: Logo UCE | Título | Avatar GEO ===
    ...
    with col_titulo:
        st.markdown("""
            <div style="padding-top: 30px;">
                ...
                <p style='margin-top: 0px; color: gray; font-size: 16px;'>Ing. GEO - Tu Tutor Virtual de la FICA</p>
            </div>
        """, unsafe_allow_html=True)

    with col_avatar_head:
        if os.path.exists(AVATAR_URL):
            # Avatar GEO a la derecha de las letras
    ...
    if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
        with contenedor_chat:
            with st.chat_message("assistant", avatar=avatar_bot):
                ...
                placeholder.markdown("🦅 *Consultando información...*")
                try:
                    textos, fuentes = leer_pdfs_locales()
                    contexto = buscar_informacion(st.session_state.messages[-1]["content"], textos, fuentes)
                    model = genai.GenerativeModel(modelo)
                    response = model.generate_content(f"Eres el Ing. GEO. Contexto: {contexto}. Pregunta: {st.session_state.messages[-1]['content']}")
